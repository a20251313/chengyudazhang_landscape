...
//	label.layer.masksToBounds = YES;
    ...
    if (type == UIInterfaceOrientationLandscapeLeft || type == UIInterfaceOrientationLandscapeRight)
    {
        CGFloat  fValue = (type == UIInterfaceOrientationLandscapeLeft?M_PI_2*3:-3*M_PI_2);
        view.transform = CGAffineTransformMakeRotation(fValue);
        overlayWindow.transform = CGAffineTransformMakeRotation(fValue);
...
-(void)changeInterface:(NSNotification*)note
{
    ...
    if (type == 4)
    {
        [view setTransform:CGAffineTransformMakeRotation(-M_PI_2*3)];
        [overlayWindow setTransform:CGAffineTransformMakeRotation(-M_PI_2*3)];
    }else if (type == 3)
    {
        [view setTransform:CGAffineTransformMakeRotation(M_PI_2*3)];
         [overlayWindow setTransform:CGAffineTransformMakeRotation(M_PI_2*3)];
